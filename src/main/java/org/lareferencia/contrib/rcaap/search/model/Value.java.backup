
/*
 *   Copyright (c) 2013-2022. LA Referencia / Red CLARA and others
 *
 *   This program is free software: you can redistribute it and/or modify
 *   it under the terms of the GNU Affero General Public License as published by
 *   the Free Software Foundation, either version 3 of the License, or
 *   (at your option) any later version.
 *
 *   This program is distributed in the hope that it will be useful,
 *   but WITHOUT ANY WARRANTY; without even the implied warranty of
 *   MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 *   GNU Affero General Public License for more details.
 *
 *   You should have received a copy of the GNU Affero General Public License
 *   along with this program.  If not, see <http://www.gnu.org/licenses/>.
 *
 *   This file is part of LA Referencia software platform LRHarvester v4.x
 *   For any further information please contact Lautaro Matas <lmatas@gmail.com>
 */

//
// This file was generated by the JavaTM Architecture for XML Binding(JAXB) Reference Implementation, v2.2.8-b130911.1802 
// See <a href="http://java.sun.com/xml/jaxb">http://java.sun.com/xml/jaxb</a> 
// Any modifications to this file will be lost upon recompilation of the source schema. 
// Generated on: 2021.03.29 at 08:39:01 AM GMT 
//


package org.lareferencia.contrib.rcaap.search.model;

import java.util.ArrayList;
import java.util.List;
import javax.xml.bind.annotation.XmlAccessType;
import javax.xml.bind.annotation.XmlAccessorType;
import javax.xml.bind.annotation.XmlAttribute;
import javax.xml.bind.annotation.XmlElement;
import javax.xml.bind.annotation.XmlRootElement;
import javax.xml.bind.annotation.XmlType;


/**
 * <p>Java class for anonymous complex type.
 * 
 * <p>The following schema fragment specifies the expected content contained within this class.
 * 
 * <pre>
 * &lt;complexType>
 *   &lt;complexContent>
 *     &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
 *       &lt;sequence>
 *         &lt;element ref="{http://www.lareferencia.info/schema/4.0/search}field-value" maxOccurs="unbounded" minOccurs="0"/>
 *       &lt;/sequence>
 *       &lt;attribute name="occurs" type="{http://www.lareferencia.info/schema/4.0/search}QuantifierEnumType" default="ONE_OR_MORE" />
 *       &lt;attribute name="condition" type="{http://www.lareferencia.info/schema/4.0/search}ConditionalOperatorEnumType" default="EQ" />
 *     &lt;/restriction>
 *   &lt;/complexContent>
 * &lt;/complexType>
 * </pre>
 * 
 * 
 */
@XmlAccessorType(XmlAccessType.FIELD)
@XmlType(name = "", propOrder = {
    "fieldValue"
})
@XmlRootElement(name = "value")
public class Value {

    @XmlElement(name = "field-value")
    protected List<String> fieldValue;
    @XmlAttribute(name = "occurs")
    protected QuantifierEnumType occurs;
    @XmlAttribute(name = "condition")
    protected ConditionalOperatorEnumType condition;

    /**
     * Gets the value of the fieldValue property.
     * 
     * <p>
     * This accessor method returns a reference to the live list,
     * not a snapshot. Therefore any modification you make to the
     * returned list will be present inside the JAXB object.
     * This is why there is not a <CODE>set</CODE> method for the fieldValue property.
     * 
     * <p>
     * For example, to add a new item, do as follows:
     * <pre>
     *    getFieldValue().add(newItem);
     * </pre>
     * 
     * 
     * <p>
     * Objects of the following type(s) are allowed in the list
     * {@link String }
     * 
     * 
     */
    public List<String> getFieldValue() {
        if (fieldValue == null) {
            fieldValue = new ArrayList<String>();
        }
        return this.fieldValue;
    }

    /**
     * Gets the value of the occurs property.
     * 
     * @return
     *     possible object is
     *     {@link QuantifierEnumType }
     *     
     */
    public QuantifierEnumType getOccurs() {
        if (occurs == null) {
            return QuantifierEnumType.ONE_OR_MORE;
        } else {
            return occurs;
        }
    }

    /**
     * Sets the value of the occurs property.
     * 
     * @param value
     *     allowed object is
     *     {@link QuantifierEnumType }
     *     
     */
    public void setOccurs(QuantifierEnumType value) {
        this.occurs = value;
    }

    /**
     * Gets the value of the condition property.
     * 
     * @return
     *     possible object is
     *     {@link ConditionalOperatorEnumType }
     *     
     */
    public ConditionalOperatorEnumType getCondition() {
        if (condition == null) {
            return ConditionalOperatorEnumType.EQ;
        } else {
            return condition;
        }
    }

    /**
     * Sets the value of the condition property.
     * 
     * @param value
     *     allowed object is
     *     {@link ConditionalOperatorEnumType }
     *     
     */
    public void setCondition(ConditionalOperatorEnumType value) {
        this.condition = value;
    }
    

    /** 
        this method was added automatically by a xjc configuration - xjc/search.xml
        any required changes, you should do on that file 
    */
    public void setFieldValue(List<String> fieldValue) {
        this.getFieldValue().removeAll(this.getFieldValue());
        this.getFieldValue().addAll(fieldValue);
    }

}
