...
public class FacetValueAssembler implements RepresentationModelAssembler<IFacetValue, FacetValue> {
    IField field;

    @Override
    public FacetValue toModel(IFacetValue facetValue) {
        FacetValue facet = new FacetValue();

        facet.setCounter(facetValue.getValueCount());
        facet.setValue(facetValue.getValue());
